      ...
      <div class="chat-main">
        ...
        <header class="chat-header">
          ...
          <div class="chat-header-right">
            ...
            <button class="header-btn header-btn--export" @click="exportConversation" :disabled="chatStore.messages.length === 0">
              <i class="fa fa-file-text-o" />
              <span>导出报告</span>
            </button>
            <button class="header-btn header-btn--feedback" @click="openFeedback(undefined)" :disabled="chatStore.messages.length === 0">
              <i class="fa fa-flag" />
              <span>反馈</span>
            </button>
          </div>
          ...
          <template v-else>
                    ...
                    <div class="assistant-bubble">
                      <template v-if="msg.segments && msg.segments.length">
                        <template v-for="(seg, i) in msg.segments" :key="i">
                          <ThinkBlock v-if="seg.type === 'think'" :content="seg.content" :streaming="false" class="mb-2" />
                      ...
                      <template v-else>
                        ...
                        <template v-if="msg.thinkBlocks && msg.thinkBlocks.length">
                          <ThinkBlock v-for="(block, i) in msg.thinkBlocks" :key="i" :content="block" :streaming="false" class="mb-2" />
                        </template>
                        <ThinkBlock v-else-if="msg.thinkContent" :content="msg.thinkContent" :streaming="false" :class="msg.content ? 'mb-2' : ''" />
            ...
            <div v-if="isStreaming" class="message-row assistant">
              <div class="assistant-msg-wrap">
                <img src="/avatar.png" alt="量仔" class="assistant-avatar" />
                <div class="assistant-content">
                  <WorkflowPanel v-if="workflowNodes.length" :nodes="workflowNodes" :running="workflowRunning" :error="workflowError" class="mb-2" />
                  <div class="assistant-bubble">
                    <template v-if="streamingParts.hasContent">
                      <template v-for="(seg, i) in streamingParts.segments" :key="i">
                        <ThinkBlock v-if="seg.type === 'think'" :content="seg.content" :streaming="!!seg.open" class="mb-2" />
        ...
        <!-- Input area -->
        <div class="input-area" @dragenter.prevent="onDragEnter" @dragover.prevent="onDragOver" @dragleave="onDragLeave" @drop.prevent="onDrop">
          <!-- Attach popup -->
          <div v-if="attachPanelOpen" class="attach-panel" ref="attachPanelRef" @click.stop>
            <div class="attach-panel-inner">
              <div class="attach-url-row">
                <input v-model="fileUrlInput" type="text" placeholder="输入文件链接" class="attach-url-input" @keypress.enter="handleUrlAttach" />
          ...
          <div class="input-row" :class="{ 'drag-over': isDragOver }">
            ...
            <div class="input-bottom-row">
              <!-- Service Switcher -->
              <div class="input-switcher-wrapper">
                <ChatServiceSwitcher @change="handleServiceChange" />
              </div>
              <input ref="inputRef" v-model="inputText" type="text" :placeholder="`和 VastAI-${appVersion} 聊天`" class="chat-input" @keypress.enter.exact="handleSend" @paste="onPaste" />
              ...
              <button class="send-btn" :class="{ active: inputText.trim() && !isStreaming }" :disabled="isStreaming || !inputText.trim()" @click="handleSend">
          ...
          <template v-else>
            <div class="feedback-dialog-body">
              ...
              <div class="feedback-rating-row">
                <button class="feedback-rating-btn" :class="{ active: feedbackRating === 'like' }" @click="feedbackRating = 'like'">
                  <i class="fa fa-thumbs-up" />
                  有帮助
                </button>
                <button class="feedback-rating-btn dislike" :class="{ active: feedbackRating === 'dislike' }" @click="feedbackRating = 'dislike'">
              ...
              <textarea v-model="feedbackContent" class="feedback-textarea" placeholder="请描述您的问题或建议..." rows="4" maxlength="500" />
            ...
            <div class="feedback-dialog-footer">
              ...
              <button class="feedback-submit-btn" :disabled="feedbackSubmitting || !feedbackRating" @click="submitFeedback">
        ...
        <div class="export-format-dialog" @click.stop>
          ...
          <div class="export-format-content">
            <div class="format-options">
              <label class="format-option" :class="{ active: selectedExportFormat === 'markdown' }" @click="selectedExportFormat = 'markdown'">
              ...
              <label class="format-option" :class="{ active: selectedExportFormat === 'html' }" @click="selectedExportFormat = 'html'">
                <div class="format-radio">
...
import { fetchConversationMessages, fetchConversations, uploadFile, saveFeedbackToDB, DEFAULT_USER_ID } from '@/api'
...
// Watch for stream conversation id
watch(streamConvId, (newId) => {
  if (newId) streamConversationId.value = newId
})
...
const streamingParts = computed(() => {
  ...
  return { segments, hasContent: segments.length > 0 }
})
...
function parseAnswer(raw: string): { beforeThink?: string; thinkContent?: string; thinkBlocks?: string[]; segments: MessageSegment[]; content: string } {
  ...
  return { beforeThink: beforeThink || undefined, thinkContent: thinkParts.length ? thinkParts.join('\n\n---\n\n') : undefined, thinkBlocks: thinkParts.length ? thinkParts : undefined, segments, content }
}
...
function getConvColor(id: string): string {
  const colors = ['#fde8e8', '#fde8f5', '#ede8fd', '#e8eafd', '#e8f4fd', '#e8fdf4', '#fdf6e8', '#fde8e8', '#e8fdfd', '#f0fde8']
  let hash = 0
...
function confirmExport() {
  const currentConv = conversations.value.find(conv => conv.id === chatStore.currentConversationId)
...
.rating-like { background: rgba(0,180,42,0.1); color: #00B42A; }
.rating-dislike { background: rgba(245,63,63,0.1); color: #F53F3F; }
.rating-null { background: #F2F3F5; color: #86909C; }
.feedback-msg-time { font-size: 11px; color: #86909C; margin-left: auto; font-weight: 400; }
