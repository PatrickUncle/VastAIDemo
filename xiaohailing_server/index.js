...
function createConnectFrame(nonce) {
  ...
  const platform = 'web';
  const deviceFamily = 'xiaohailing';
  ...
  return {
    ...
    params: {
      ...
      client: { id: 'gateway-client', displayName: '小海灵', version: '1.0.0', platform, deviceFamily, mode: 'backend' },
      role: 'operator',
      scopes: SCOPES,
      caps: [],
      auth,
...
function createNodeConnectFrame(nonce) {
  ...
  const deviceFamily = 'xiaohailing-node';
  // nonce 必须非空（NonEmptyString），且签名与 device.nonce 必须一致
  const effectiveNonce = nonce || randomUUID();
  ...
  return {
    ...
    params: {
      minProtocol: 3, maxProtocol: 3,
      client: { id: 'node-host', displayName: '小海灵 Node', version: '1.0.0', platform, deviceFamily, mode: 'node' },
      role: 'node',
      scopes: [],
      caps: ['system'],
      ...
      locale: 'zh-CN',
      userAgent: 'Xiaohailing-Node/1.0.0',
    },
  };
}
...
function handleUpstreamMessage(sid, rawData) {
  ...
  if (session.state === 'connected') {
    ...
    if (msg.type === 'event') {
      ...
      // node 设备配对请求 → 自动审批（允许小海灵 node 设备接受 system.notify）
...
app.post('/api/connect', async (req, res) => {
  ...
  try {
    ...
    if (/ECONNREFUSED/.test(userError)) {
      ...
      userError = 'Gateway 认证失败，请检查 xiaohailing_server/.env 中的 OPENCLAW_GATEWAY_TOKEN 配置';
...
server.listen(CONFIG.port, () => {
  log.info(`小海灵代理服务端已启动: http://0.0.0.0:${CONFIG.port}`);
  ...
  if (_isFirstRun) {
    ...
    log.warn('未设置连接密码 (PROXY_TOKEN)，请在 xiaohailing_server/.env 中配置');
  }
